...
def reset_variables():
    global failure_count, uptime_start_time, downtime_start_time, last_insert_time1, production_time
    failure_count = 0
...
def main():
    ...
    while True:
        ...
        target_time = Current_time_1.replace(hour=11, minute=34, second=0, microsecond=0)

        # Define a time window of 1 minute
        time_window_start = target_time - datetime.timedelta(minutes=1)
        time_window_end = target_time + datetime.timedelta(minutes=1)

        if time_window_start <= Current_time_1 <= time_window_end and not data_inserted:
