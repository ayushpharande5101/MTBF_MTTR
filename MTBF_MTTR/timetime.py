    ...
    while True:
        ...
        if Z.registers[0] == 1:
            ...
            while True:
                ...
                A6.append(TOTAL_PRODUCTION_TIME/60)
                TOTAL_PRODUCTION_TIME = A6[-1]
                print(f"TOTAL_PRODUCTION_TIME",TOTAL_PRODUCTION_TIME)
                Z = client.read_holding_registers(0, 5)
                ...
                if Z.registers[0] == 1:
                    ...
                    START_TIME += 1
                    A.append(CURRENT_TIME/60)

                    if len(A) >= 1:
                        x = len(A) - 1
                        TOTAL_UPTIME = A[x] - 0
                        print(f"TOTAL_UPTIME == {TOTAL_UPTIME} min")
                    ...
                    START_TIME1 += 1
                    B.append(CURRENT_TIME/60)

                    if len(B) >= 1:
                        y = len(B) - 1
                        TOTAL_DOWN = B[y] - 0
                        print(f"TOTAL_DOWNTIME == {TOTAL_DOWN} min")
                ...
                if current_time - last_insert_time >= 5:   #122.76:  # Check if 10 minutes (600 seconds) have passed
